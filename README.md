# SelfLPDirect: Auto-Compounding Hook for Uniswap V4

A Foundry workshop scaffold demonstrating **custom-accounting delta management** in Uniswap V4 hooks. The hook owns and auto-compounds its own concentrated LP position, reinvesting accumulated fees without an NFT or PositionManager.
...
The core innovation is **flash accounting** — netting multiple deltas without physical token movement until the final settlement.
...
# Run specific test
forge test --match-test seedPosition_Success -vvv

# Run with gas report
forge snapshot
```

**Test file:** `test/SelfLPDirect.t.sol`

Key scenarios:
- ✓ Pool initialization (dynamic-fee + ETH guard)
- ✓ Position seeding
- ✓ Fee accumulation & threshold trigger
- ✓ Reinvest cycle (burn → mint → settle)
- ✓ Edge cases (no liquidity after burn, tiny positions)
...
```
src/
  ├── SelfLPDirect.sol          # Baseline hook (this one)
  └── lib/
      └── SelfLPLib.sol         # Shared math: computeRange, previewFeesETH, computeReinvestSwap

test/
  ├── SelfLPDirect.t.sol        # Full test suite
  └── hlpEnvelopTest.sol        # Logging helpers

CLAUDE.md                        # Developer guidance
AGENTS.md                        # Task file location
```
