...
/// @title SelfLPAfterDelta — auto-compounding self-LP hook with afterSwapReturnDelta skimming.
/// @notice Extends SelfLPDirect with a skim buffer: collects a percentage (SKIM_BPS) of the
///         unspecified-side amount on every swap via `afterSwapReturnDelta`, and merges the
///         accumulated skim into the position at each reinvest.
///
/// @dev This demonstrates the **afterSwapReturnDelta** hook pattern:
///        • For every swap, skim a fixed bps from the unspecified side (the output side for
///          exactIn, or input side for exactOut).
///        • Physically take the skim amount via `poolManager.take()` (flash-accounting debit).
///        • Return `hookDeltaUnspecified > 0` so the PoolManager nets the debit against the
///          swapper's output.
///        • On reinvest, the skimmed tokens are already in `balanceOfSelf()` and folded into
///          the new position via the standard flash-accounting flow.
///
///      The `skimBuffer0` and `skimBuffer1` storage fields are **observational**: they track
///      cumulative skims between swaps for test inspection, but correctness doesn't depend on
///      them — `balanceOfSelf()` captures all tokens including skim.
contract SelfLPAfterDelta is BaseHook, IUnlockCallback {
    ...
    /// @notice Basis points to skim from unspecified side on every swap (e.g. 100 = 1%).
    uint16 public immutable SKIM_BPS;
    ...
    // -----------------------------------------------------------------------
    // Skim buffers — observational (correctness uses balanceOfSelf)
    // -----------------------------------------------------------------------

    uint128 public skimBuffer0;
    uint128 public skimBuffer1;
    ...
    /// @notice Emitted when a skim is collected.
    /// @param currency Either currency0 or currency1 (the unspecified side).
    /// @param amount Amount skimmed.
    event SkimCollected(Currency indexed currency, uint128 amount);
    ...
    // -----------------------------------------------------------------------
    // After-swap: skim + threshold gate + reinvest
    // -----------------------------------------------------------------------
    ...
    function _afterSwap(
        ...
        // Determine which side is unspecified (the output of exactIn or input of exactOut).
        // unspecified = currency1 if (exactIn && zeroForOne) or (exactOut && !zeroForOne).
        ...
        // Calculate and take the skim.
        uint128 skimAmount = uint128(unspecAmount < 0 ? -unspecAmount : unspecAmount) * SKIM_BPS / 10_000;

        if (skimAmount > 0) {
            // Physically move skim to hook (flash-accounting debit on hook).
            poolManager.take(unspecCurrency, address(this), skimAmount);

            // Accumulate in buffer for observability.
            ...
            emit SkimCollected(unspecCurrency, skimAmount);
        }
        ...
        if (feesEth < feeThresholdETH) {
            // No reinvest, but return the skim delta so PoolManager nets it against swapper.
            return (this.afterSwap.selector, int128(skimAmount));
        }

        // Reinvest: merge skim buffer + accrued fees into new position.
        _reinvest(key, sqrtPriceX96);

        // After reinvest, buffers are merged into the position, so reset them.
        skimBuffer0 = 0;
        skimBuffer1 = 0;
    ...
    ///        3. Compute target liquidity from `(burnDelta amounts) + idle balance` (which now
    ///           includes skimmed tokens from take() above).
    ...
    function _reinvest(PoolKey calldata key, uint160 sqrtPriceX96) internal {
        ...
        //    The idle balance now includes skimmed tokens from take() in _afterSwap.
